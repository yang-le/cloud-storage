...
\begin{definition}
设$V$是$\mathbb{R}$上的有限维矢量空间。线性映射$\omega \colon V \to \mathbb{R}$称为$V$上的对偶矢量。
$V$上全体对偶矢量的集合称为$V$的对偶空间，记作$V^*$。
\end{definition}
...
因此$V^*$中任一元素可用$\{e^{\mu*}\}$线性表出，故$\{e^{\mu*}\}$是$V^*$的一个基底，叫做$\{e_\mu\}$的对偶基底。于是可知$\dim V^* = \dim V$。
\end{proof}

两个矢量空间叫同构的，若两者之间存在一一到上的线性映射（这种映射称为同构映射）。两矢量空间同构的充要条件是维数相同。
...
若在$M$（或$A \in M$）上每点指定一个对偶矢量，就得到$M$（或$A$）上的一个对偶矢量场。
$M$上的对偶矢量场$\omega$叫做光滑的，若对所有光滑矢量场$v$有$\omega(v) \in \mathscr{F}_M$。
