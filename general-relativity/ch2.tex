...
\begin{definition}
矢量空间$V$上的一个度规$g$是$V$上的一个对称、非退化的$(0, 2)$型张量。
...
\begin{definition}
$v \in V$的长度或大小定义为$|v| := \sqrt{|g(v, v)|}$。矢量$v, u \in V$叫相互正交的，若$g(v, u) = 0$。
$V$的基底$\{e_\mu\}$叫正交归一的，若任二基矢正交且每一基矢$e_\mu$满足$g(e_\mu, e_\mu) = \pm 1$（不对$\mu$求和）。
\end{definition}
...
\begin{definition}
用正交归一基底写成对角矩阵后，对角元全为$+1$的度规叫正定的或黎曼的，
对角元全为$-1$的度规叫负定的，其他度规叫不定的，只有一个对角元为$-1$的不定度规叫洛伦兹的。
对角元之和叫度规的号差。相对论中用得最多的是洛伦兹度规和正定度规。
\end{definition}

\begin{definition}
带洛伦兹度规$g$的矢量空间$V$的元素可分为三类：
\textcircled{1}满足$g(v, v) > 0$的$v$称为类空矢量；
\textcircled{2}满足$g(v, v) < 0$的$v$称为类时矢量；
\textcircled{3}满足$g(v, v) = 0$的$v$称为类光矢量。
\end{definition}
...
\begin{definition}
$M$上的对称的、处处非退化的$(0, 2)$型张量场称为度规张量场。
\end{definition}
...
若$C^1$曲线$C(t)$各点的切矢都类空，则$C(t)$叫类空曲线。
类似地可定义类时曲线和类光曲线。
...
$l$也可充当该线的参数，称为线长参数。由$dl \equiv \sqrt{|g(T, T)|}\diff t$可知，以线长为参数的曲线切矢满足$|g(T, T)| = 1$，即有单位长。

\begin{definition}
设流形$M$上给定度规场$g$，则$(M, g)$叫广义黎曼空间（若$g$为正定，叫黎曼空间；若$g$为洛伦兹，叫伪黎曼空间，物理上叫时空。）
\end{definition}
...
则$(\mathbb{R}^n, \delta)$称为$n$维欧氏空间，$\delta$称为欧氏度规。
\end{definition}
...
的坐标系叫笛卡尔坐标系或直角坐标系。换句话说，一个坐标系叫笛卡尔系，若其坐标基底用欧氏度规$\delta$衡量为正交归一。
\end{definition}
