...
$\mathrm{d}x^\mu$是$O$上的第$\mu$个对偶坐标基矢场，$\{\mathrm{d}x^\mu\}$则是$O$上的第$\mu$个对偶坐标基底场。
...
准确地说，$(M, g)$称为时空，\CJKunderdot{若}$M$为\CJKunderdot{连通}流形，$g$为有足够可微程度的洛伦兹度规场。
}）。
\end{definition}
...
由定义可知闵氏度规在自然坐标系的线元表达式为$\mathrm{d}s^2 = \eta_{\mu\nu}\mathrm{d}x^\mu\mathrm{d}x^\nu$。
