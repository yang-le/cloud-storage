...
\begin{definition}
$N \subset X$称为$A \subset X$的一个\emph{邻域}，若存在$O \in \mathscr{T}$使$A \subset O \subset N$。
\end{definition}
...
\begin{definition}
$C \subset X$叫\emph{闭集}，若$-C \in \mathscr{T}$。
\end{definition}
...
\begin{definition}
拓扑空间$(X, \mathscr{T})$称为\emph{连通}的，若它除$X$和$\emptyset$外没有既开又闭的子集。
\end{definition}
...
\begin{definition}
$A$的\emph{闭包}$\bar{A}$是所有含$A$的闭集的交集，即
$$\bar{A} := \bigcap_\alpha C_\alpha, ~ A \subset C_\alpha, ~ -C_\alpha \in \mathscr{T}$$
\end{definition}

\begin{definition}
$A$的\emph{内部}$\interior(A)$是所有含于$A$的开集的并集，即
$$\interior(A) := \bigcup_\alpha O_\alpha, ~ O_\alpha \subset A, ~ O_\alpha \in \mathscr{T}$$
\end{definition}

\begin{definition}
$A$的\emph{边界}$\dot{A} := \bar{A} - \interior(A)$，$x \in \dot{A}$称为\emph{边界点}。$\dot{A}$也记作$\partial A$
\end{definition}
...
\begin{definition}
$X$的开子集的集合$\{O_\alpha\}$叫$A \subset X$的一个\emph{开覆盖}，若$A \subset \bigcup\limits_\alpha O_\alpha$。
也可以说$\{O_\alpha\}$覆盖$A$。
\end{definition}
