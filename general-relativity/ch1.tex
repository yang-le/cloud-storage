...
$$\operatorname{i}(A) := \bigcup_\alpha O_\alpha, ~ O_\alpha \subset A, ~ O_\alpha \in \mathscr{T}$$
\end{definition}

\begin{definition}
$A$的\textbf{边界}$\dot{A} := \bar{A} - \operatorname{i}(A)$，$x \in \dot{A}$称为\textbf{边界点}。$\dot{A}$也记作$\partial A$
\end{definition}
